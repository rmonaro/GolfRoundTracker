...
function WeekStat({ label, value }: { label: string; value: string }) {
  return (
    <Box sx={{ textAlign: 'center', px: 1 }}>
      <Typography sx={{ fontWeight: 800, fontSize: '1.4rem', lineHeight: 1.15 }}>
        {value}
      </Typography>
      <Typography
        variant="caption"
...
export function PracticeStartPage() {
  ...
  const week = useMemo(() => {
    ...
    const tempos = sessions.map((s) => s.avgTempoRatio).filter((v): v is number => v != null);
    const cons = sessions.map((s) => s.tempoConsistencyScore).filter((v): v is number => v != null);
  ...
  return (
    <Box sx={practicePageSx()}>
      <IconButton aria-label="Back" onClick={() => navigate('/')} sx={{ ml: -1, mb: 1 }}>
      ...
      <Typography variant="body2" color="text.secondary" sx={{ mt: 2 }}>
        ...
        <strong>tempo</strong> and <strong>consistency</strong>. It is not a launch
        monitor and does not measure ball flight, club path, or swing-plane angle.
      </Typography>
          ...
          <CardContent sx={{ textAlign: 'center', py: 4 }}>
            ...
            >
              <svg width="26" height="26" viewBox="0 0 22 22" fill="none">
                <circle cx="11" cy="11" r="7.5" stroke="#ffd580" strokeWidth="1.5" />
                <circle cx="11" cy="11" r="3" stroke="#ffd580" strokeWidth="1.5" />
                <circle cx="11" cy="11" r="1" fill="#ffd580" />
              </svg>
            </Box>
            <Typography sx={{ fontWeight: 700, fontSize: '18px' }}>No practices yet</Typography>
            <Typography
              color="text.secondary"
              sx={{ fontSize: '13px', lineHeight: 1.5, mt: 0.5, maxWidth: 300, mx: 'auto' }}
            >
              Take your first swing to start tracking tempo, consistency, and your shots by club.
            </Typography>
          </CardContent>
        </Card>
      ) : (
        <Card elevation={0} sx={{ bgcolor: 'background.paper', borderRadius: '5px', mt: 3 }}>
          <CardContent>
            <Stack direction="row" spacing={4} alignItems="flex-start">
              <Box>
                ...
                <Typography sx={{ fontSize: '3em', fontWeight: 800, lineHeight: 1.1 }}>
                  {week.thisWeekCount}
                </Typography>
              </Box>
              <Box>
          ...
          <Box sx={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', mt: 2 }}>
            {week.days.map((d, i) => (
              <Stack key={i} sx={{ alignItems: 'center', flex: 1 }} spacing={0.75}>
                <Box sx={{ height: 48, display: 'flex', alignItems: 'flex-end' }}>
                  <Box
                    sx={{
                      width: 10,
                      ...
                      bgcolor: d.isToday
                        ? '#f88930'
                        : (theme) => (theme.palette.mode === 'dark' ? '#3b475e' : '#cfd6e0')
                    }}
                  />
                </Box>
                <Typography variant="caption" color="text.secondary">
          ...
          <Stack direction="row" divider={<Divider orientation="vertical" flexItem />}>
            <Box sx={{ flex: 1 }}>
              <WeekStat
                label="Avg Tempo"
                value={week.avgTempo != null ? `${week.avgTempo.toFixed(1)}:1` : '—'}
              />
            </Box>
            <Box sx={{ flex: 1 }}>
            ...
            <Box sx={{ flex: 1 }}>
              <WeekStat
                label="Consistency"
                value={week.consistency != null ? `${Math.round(week.consistency)}%` : '—'}
              />
            </Box>
          </Stack>
          </CardContent>
      ...
      >
        ...
        <List sx={{ pt: 1, px: 0 }}>
          ...
          <ListItemButton disabled sx={practiceItemSx}>
            <Box sx={practiceIconBoxSx}>
              <svg width="22" height="22" viewBox="0 0 22 22" fill="none">
                <path d="M8 3.2v9.5" stroke="#8a93ab" strokeWidth="1.7" strokeLinecap="round" />
                <path d="M8 3.2l5.5 1.7L8 6.6Z" fill="#8a93ab" />
                <ellipse cx="11" cy="16.6" rx="7" ry="2.6" stroke="#8a93ab" strokeWidth="1.5" />
              </svg>
            </Box>
            <ListItemText
              primary="Putting"
